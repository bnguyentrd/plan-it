...
const PollDetail = () => {
  ...
  const fetchData = async () => {
    console.log("POLL DETAIL LINE 54 IN FETCH DATA", id);
    const questionResponse = await fetch(
      `${process.env.REACT_APP_POLLS_API_HOST}/questions/${id}`
    );
    const questionData = await questionResponse.json();
    setQuestion(questionData);

    const choicesResponse = await fetch(
      `${process.env.REACT_APP_POLLS_API_HOST}/choices`
    );
  ...
  return (
    <>
      ...
      <div>
        ...
        <h3>Status: {question.is_active}</h3>
        <div>
        ...
        <button onClick={handleNavChoices}>
          Create a Choice for this Poll
        </button>
      </div>
    </>
  );
